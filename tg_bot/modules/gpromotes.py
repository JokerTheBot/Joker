...
@run_async
def revokesudo(bot: Bot, update: Update, args: List[str]):
    message = update.effective_message
    user_id = extract_user(message, args)
    user_chat = bot.get_chat(user_id)
    if not user_id:
...
GPROMOTE_HANDLER = CommandHandler("promotetosudo", promotetosudo, pass_args=True, filters=Filters.user(OWNER_ID))
UNGPROMOTE_HANDLER = CommandHandler("revokesudo", revokesudo, pass_args=True, filters=Filters.user(OWNER_ID))
dispatcher.add_handler(PROMOTETOSUDO_HANDLER)
dispatcher.add_handler(REVOKESUDO_HANDLER)
